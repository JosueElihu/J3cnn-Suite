...
Begin VB.Form Form2 
   ...
   Caption         =   "JSqlite.connector - BLOB'S"
   ClientHeight    =   4185
   ClientLeft      =   45
   ClientTop       =   390
   ClientWidth     =   6015
...
Dim oPic    As StdPicture
    ...
    Set Query = dbc.Query("SELECT * FROM employees;")
    ...
    Do While Query.Step = SQLITE_ROW
        ...
        ' OBTENER ARRAY DE BYTES DE LA IMAGEN
        '--------------------------------------------------------------------
        ' - Query.Blob(3)
        ' - Query.Blob("profile")
        ' - Query.Value(3, SQLITE_BLOB)
        ' - Query.Value("profile", SQLITE_BLOB)
        '--------------------------------------------------------------------
        
        ' Usaremos gdi+ para convertir el array de bytes de la imagen en un
        ' Objeto StdPicture y añadir a la una coleccion para luego mostrarlo
        ' En un control Image
        
        Set oPic = ArrayToPicture(Query.Blob("profile"))
        Pics.Add oPic
